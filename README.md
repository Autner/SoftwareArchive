...
**代码与数据分离**：App 本体只包含工具代码。首次运行会把工具安装到 `~/SoftwareArchive`，收录的软件档案保存在其中的 `Library`。升级 App 时只更新 `~/SoftwareArchive` 里的代码文件，档案数据不受影响；删除重装 App 同样不影响数据。可用环境变量 `SA_HOME` 指定其他数据目录。
